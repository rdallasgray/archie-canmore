...
(function() {
  ...
  Architect = (function() {
    ...
    Architect.prototype.CANMORE_REQUEST_URL = 'http://glowing-moon-5208.heroku.com/';
    ...
    function Architect(canmoreRequestUrl) {
      ...
      this.lastLocation = new window.AR.GeoLocation(0, 0, 0);
      this.currentLocation = new window.AR.GeoLocation(0, 0, 0);
      this.geoObjects = {};
    ...
    Architect.prototype.createARImage = function(item) {
      ...
      if (!this.geoObjects[item]) {
        return this.serverRequest("details_for/", [item], function(item_details) {
          ...
          location = new window.AR.GeoLocation(item_details.lat, item_details.long, _this.currentLocation.altitude);
          ...
          _this.geoObjects[item] = new window.AR.GeoObject(location, {
            enabled: false
          });
    ...
    Architect.prototype.createImageResource = function(uri, geoObject) {
      var imgRes,
        _this = this;
      imgRes = new window.AR.ImageResource(uri, {
        onError: function() {
    ...
    Architect.prototype.createImageDrawable = function(imgRes, options) {
      return new window.AR.ImageDrawable(imgRes, this.DEFAULT_HEIGHT_SDU, options);
    };

    Architect.prototype.serverRequest = function(url, params, callback) {
      params || (params = []);
      return $.getJSON(this.CANMORE_REQUEST_URL + url + params.join('/') + '?callback=?', function(data) {
  ...
  architect = new Canmore.Architect;

  $(function() {
    return window.AR.context.onLocationChanged = function(lat, long, alt, acc) {
      return architect.locationChanged(lat, long, alt, acc);
    };
  });
